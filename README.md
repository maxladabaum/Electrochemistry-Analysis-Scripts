...
- Every entry in **Channels to combine** receives one stable response color from the complete option list: a distinct orange shade for signal-on or a distinct blue shade for signal-off. Neighboring Method 1 / Method 2 options alternate between deliberately separated light and dark regions of their color family, keeping them legible even in large palettes and translucent scatter plots. That exact channel-to-color mapping is reused in combined and individual metrics, plateau plots, Langmuir fits, SNR, concentration diagnostics, drift plots, and figure/PDF exports; selecting a subset or changing diagnostic grouping never reassigns shades. Direction comes from the Langmuir amplitude when available and otherwise from the median selected target-minus-preceding-buffer plateau change, so Langmuir plotting is not required. SWV methods use the same marker and line style and are differentiated by their shades. Combined mixed-direction iteration plots use one shared buffer-relative current axis and a zero reference line rather than separate y-axes.
...
- Titration plateau plots mirror their corresponding iteration plots: combined peak-current and wavelet-energy plateaus use the same anchor-buffer translation and zero reference, while every plateau metric retains the orange signal-on / blue signal-off channel colors. Plateau axes for other metrics remain unchanged.
...
- Each diagnostic group also receives a concentration-versus-measurement-number plot. Individual target and buffer SWV responses are converted to concentration by inverting that group's fitted Langmuir curve, then plotted against the original/source SWV measurement sequence (with local scan number as fallback). The original titration boundaries are overlaid as lightly labeled vertical dashed lines in that same measurement coordinate system. This plot uses the uncensored inverse-Langmuir estimate rather than flooring predictions at LOD, so positive values below LOD and negative apparent concentrations remain visible; table values and the other concentration diagnostic retain their existing LOD reporting. Its y coordinates use doubling levels internally: zero concentration is `0`, the lowest selected nonzero dose is `1`, and `y = 1 + log2(C / Cmin)` above that dose, so every concentration doubling occupies exactly one vertical interval. Concentrations at or below the lowest dose use the monotonic linear mapping `y = C / Cmin`, which extends through zero for negative estimates. The visible y-axis is labeled only as concentration, with ticks `0` and the original selected concentration values—transformed level numbers are not shown. Uncertainty endpoints are transformed separately to retain asymmetric intervals. The default upper limit is the transformed value of `1.3 ×` the largest selected concentration; the lower limit expands to include uncensored predictions. Open that graph's **Plot settings**, enable **Manual y-axis limits**, and enter doubling-level limits to override its range independently; the downloaded image uses the same limits. The known titration is drawn as a step reference with one shaded ±20% band. A compact subtitle explicitly reports RMSE across all displayed predicted-versus-known measurements and the percentage of nonzero target measurements within ±20%; the long uncertainty-status note is kept out of the data area. Both this plot and predicted-versus-known concentration use orange channel/method markers for signal-on fits and blue markers for signal-off fits; reference and acceptance-region colors remain unchanged.
